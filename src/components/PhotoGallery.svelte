<script lang="ts">
  ...
  let displayedPhotos = $state(getRandomizedPhotos())
...
<div class="photo-gallery" onclick={handleClick} role="button" tabindex="0">
  {#each displayedPhotos as photo (photo.path)}
    <div
      class="photo-item"
      style="grid-column: {photo.gridColumn}; grid-row: {photo.gridRow}"
    >
